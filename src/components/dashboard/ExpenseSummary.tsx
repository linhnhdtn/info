...
export function ExpenseSummary() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/budgets?month=${monthKey}`).then((r) => r.json()),
      fetch(`/api/expenses?month=${monthKey}`).then((r) => r.json()),
    ])
      .then(([b, e]) => {
        setBudget(b)
        setExpenses(e)
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Link href="/expenses" className="text-xs text-blue-600 hover:underline">Chi tiết →</Link>
      </CardHeader>
      <CardContent className="space-y-3">
        {!budget ? (
          <p className="text-sm text-muted-foreground text-center py-2">
            <Link href="/expenses" className="text-blue-600 hover:underline">Thiết lập ngân sách →</Link>
          </p>
        ) : (
          <>
